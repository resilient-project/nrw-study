...
"""
Create summary CSV files for all scenario runs restricted to Germany,
combining individual per-network CSVs into multi-scenario DataFrames.
"""
...
if __name__ == "__main__":
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake("make_global_summary_de")

    configure_logging(snakemake)
    set_scenario_config(snakemake)

    for kind in snakemake.output.keys():
        logger.info(f"Creating global DE summary for {kind}")
